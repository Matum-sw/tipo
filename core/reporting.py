...
def _app_context_for_prompt() -> list[str]:
    return [
        ...
        "- `AI 시간표 재조정`: 직접 재조정 또는 자동 재조정 방식을 선택합니다.",
        "- `직접 재조정`: 아래 데이터를 바탕으로 GPT에 붙여넣을 개인화 재조정 프롬프트를 복사합니다.",
        "- `자동 재조정`: OpenAI API 키가 있으면 최근 사용 패턴을 바탕으로 현재 시간 이후 계획을 재조정합니다.",
        "",
        "## 타이머와 휴식 규칙",
...
def build_ai_coaching_prompt(day: str, snapshots: list[dict], current_context: dict | None = None) -> str:
    ...
    lines = [
        "# 개인화 AI 피드백 요청 프롬프트",
        "",
        "아래 데이터는 내 로컬 플래너에서 직접 재조정으로 복사한 사용 기록입니다.",
